...
/// <summary>
/// SignalR-хаб, обеспечивающий обмен сообщениями между пользователями,
/// обработку обращений в службу поддержки, взаимодействие с ботом поддержки
/// и подключение администраторов к чатам.
/// </summary>
public class ChatHub : Hub
{
    ...
    /// <summary>
    /// Отправляет сообщение в чат, сохраняет его в базе данных,
    /// обрабатывает вложения, уведомляет собеседника по электронной почте
    /// и при необходимости инициирует работу бота поддержки.
    /// </summary>
    ...
    public async Task SendMessage(int chatId, string userName, string message, AttachmentDto[] attachments)
    {
        ...
        { 
            ...
            if (escalate) 
            { 
                var escalationMessage = await _context.Messages
    ...
    /// соответствующей указанному чату.
    /// </summary>
    /// <param name="chatId">Идентификатор чата.</param>
    /// <returns>Асинхронная задача.</returns>
    ...
    /// <summary>
    /// Подключает администратора к группе администраторов
    /// для получения уведомлений о новых обращениях в поддержку.
    /// </summary>
    /// <returns>Асинхронная задача.</returns>
    ...
    /// <summary>
    /// Удаляет текущего администратора из группы администраторов.
    /// </summary>
    /// <returns>Асинхронная задача.</returns>
    ...
    /// <summary>
    /// Передаёт чат администратору,
    /// отключая автоматические ответы бота поддержки.
    /// </summary>
    /// <param name="chatId">Идентификатор чата.</param>
    /// <returns>Асинхронная задача.</returns>
    ...
    /// <summary>
    /// Завершает работу администратора с чатом
    /// и повторно активирует бота поддержки.
    /// </summary>
    /// <param name="chatId">Идентификатор чата.</param>
    /// <returns>Асинхронная задача.</returns>
